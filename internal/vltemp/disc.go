...
type DiscWFID struct {
	volumeWFID VolumeWFID
	discDirName string
	valid bool
}

var ErrInvalidDiscDirName = errors.New("invalid disc directory name")

func NewDiscWFID(volumeWFID VolumeWFID, discDirName string) (DiscWFID, error) {
	if discDirName == "" || strings.Contains(discDirName, "/") {
		return DiscWFID{}, ErrInvalidDiscDirName
	}
	return DiscWFID{
		volumeWFID: volumeWFID,
		discDirName: discDirName,
		valid: true,
	}, nil
}
...
func NewDiscWFIDFromString(asString string) (DiscWFID, error) {
	...
	if len(parts) != 2 {
		return DiscWFID{}, ErrInvalidDiscWFIDString
	}
	volumeWFID, err := NewVolumeWFID(parts[0])
	if err != nil {
		return DiscWFID{}, err
	}
	return NewDiscWFID(volumeWFID, parts[1])
}

func (id *DiscWFID) checkValid() {
	if !id.valid {
		panic("invalid DiscWFID")
	}
}

func (id *DiscWFID) String() string {
	id.checkValid()
	return filepath.Join(string(id.volumeWFID), id.discDirName)
}
...
func (id *DiscWFID) DiscDirName() string {
	id.checkValid()
	return id.discDirName
}